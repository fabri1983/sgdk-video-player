...
static FORCE_INLINE void unpackFrameTileset (TileSet* src)
{
	...
	if (src->compression != COMPRESSION_NONE) {
        ...
        memcpy_asm(lenBytes, FAR_SAFE(src->tiles, lenBytes), (u8*) unpackedTilesetChunk);
}
...
static FORCE_INLINE void unpackFrameTilemap (TileMapCustomCompField* src)
{
	...
	if (src->compression != COMPRESSION_NONE) {
        ...
        memcpy_tilemap_extended_asm(FAR_SAFE(src->tilemap, lenBytes), (u8*) unpackedTilemap);
        #else
        // if NOT using extended width in the data generated by rescomp we do the copy in one go
        memcpy_tilemap_asm(FAR_SAFE(src->tilemap, lenBytes), (u8*) unpackedTilemap);
        #endif
	}
}
...
{
    // Same than allocating when we use RAM_FIXED_MOVIE_FRAME_UNPACKED_TILEMAP_ADDRESS
    allocateTilemapBuffer();
}

static void allocatePalettesBuffer ()
{
	unpackedPalsRender = (u16*) MEM_alloc(VIDEO_FRAME_PALS_COLORS_NUM * 2);
	unpackedPalsBuffer = (u16*) MEM_alloc(VIDEO_FRAME_PALS_COLORS_NUM * 2);
}

static FORCE_INLINE void unpackFramePalettes (u16 compression, u16* data, u16 len, u16 offset)
{
#if MOVIE_FRAME_STRIPS > 1
	...
	if (compression != COMPRESSION_NONE) {
        ...
        //memcpy((u8*) (unpackedPalsBuffer + offset), data, lenBytes);
        memcpy_asm(lenBytes, (u8*)data, (u8*)(unpackedPalsBuffer + offset));
    }
#else
    // Copy the palette data. No FAR_SAFE() needed here because palette data is always stored at NEAR region by rescomp
	const u16 lenBytes = len * 2;
	//memcpy((u8*) (unpackedPalsRender + offset), data, lenBytes);
    memcpy_asm(lenBytes, (u8*)data, (u8*)(unpackedPalsRender + offset));
#endif
}
...
static FORCE_INLINE void queueTilemapData ()
{
	...
	// Now we use custom DMA_queueDmaFast() because the data is in RAM, so no 128KB bank boundary check is needed
	DMA_ELEMS_queue(RAM_FIXED_MOVIE_FRAME_UNPACKED_TILEMAP_ADDRESS, VIDEO_FRAME_PLANE_ADDRESS + ((0 & TILE_INDEX_MASK) * 2), 
        lenInWords, VIDEO_PLAYER_DMA_ELEM_TYPE_TILEMAP);
}
...
void playMovie ()
{
    ...
    {
        memsetU16(unpackedPalsRender, 0x0, VIDEO_FRAME_PALS_COLORS_NUM); // black all the buffer
		...
		SYS_disableInts();
            ...
            VDP_setHInterrupt(TRUE);
                ...
                // But introduces more pression over the m68k bus so the PCM playback syncs out a bit.
		...
		while (vFrame < MOVIE_FRAME_COUNT)
		{
			...
			#if VIDEO_FRAME_ADVANCE_STRATEGY == 1 // Takes 202~235 cycles with a peak of 657 (why?). HInt disabled.
			vFrame = divu(hwFrameCntr * MOVIE_FRAME_RATE, sysFrameRate);
			#elif VIDEO_FRAME_ADVANCE_STRATEGY == 2 // Takes 370~406 cycles with a peak of 865 (why?). HInt disabled.
			vFrame = (hwFrameCntr * sysFrameRateReciprocal) >> 16;
			#elif VIDEO_FRAME_ADVANCE_STRATEGY == 3 // Takes 204~235 cycles with a peak of 687 (why?). HInt disabled.
			...
			#elif VIDEO_FRAME_ADVANCE_STRATEGY == 4 // Takes 71~93 cycles with a peak of 534 (why?). HInt disabled.
			...
			KLog_U1("", frmCntr - prevFrame); // this tells how many system frames are spent for unpack, dma, display, etc per video frame
			#endif
