...
STOPWATCH_68K_CYCLES_START();
	// NOTE: if NOT using RLEWXMAP enable next
	...
	// NOTE: if using RLEWXMAP enable next
	...
	rlewxmap_decomp_B_asm(jumpGap, (u8*) FAR_SAFE(src->tilemap, size), (u8*) unpackedTilemap);
STOPWATCH_68K_CYCLES_STOP();
	...
	if (src->compression != COMPRESSION_NONE) {
		...
		// const u8 jumpGap = 2 * (MOVIE_FRAME_EXTENDED_WIDTH_IN_TILES - MOVIE_FRAME_WIDTH_IN_TILES);
		// const u16 size = MOVIE_FRAME_WIDTH_IN_TILES * MOVIE_FRAME_HEIGHT_IN_TILES * 2;
		// rlewxmap_decomp_B(jumpGap, (u8*) FAR_SAFE(src->tilemap, size), (u8*) unpackedTilemap);
	}
	else {
		// NOTE: if using extended width in the data generated by rescomp then enable next
		const u16 size = MOVIE_FRAME_EXTENDED_WIDTH_IN_TILES * MOVIE_FRAME_HEIGHT_IN_TILES * 2;
    ...
    // u16* ptrAux = unpackedTilemap;
    // for (u8 ii=MOVIE_FRAME_HEIGHT_IN_TILES; ii--;) {
    //     for (u8 jj=MOVIE_FRAME_WIDTH_IN_TILES; jj--;)
    //         kprintf("%02X, ", *ptrAux++);
	// 	ptrAux += MOVIE_FRAME_EXTENDED_WIDTH_IN_TILES - MOVIE_FRAME_WIDTH_IN_TILES;
    // }
    // kprintf("################################################");
}
