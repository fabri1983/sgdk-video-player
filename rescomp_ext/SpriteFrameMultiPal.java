...
import sgdk.rescomp.resource.TilesetForSpriteMultiPal;
...
public class SpriteFrameMultiPal extends Resource
{
    public final List<VDPSpriteMultiPal> vdpSprites;
    public final Collision collision;
    public final TilesetForSpriteMultiPal tileset;
    ...
    public SpriteFrameMultiPal(String id, byte[] frameImage8bpp, int wf, int hf, int timer, CollisionType collisionType, Compression compression,
    		List<SpriteCell> sprites, SpriteBoundariesPalettes spriteBoundariesPals)
    {
        super(id);
        ...
        if (sprites.isEmpty())
        {
            ...
            tileset = (TilesetForSpriteMultiPal) addInternalResource(new TilesetForSpriteMultiPal(id + "_tileset", false));
        }
        else
        {
            int optNumTile = 0;
            ...
            // build tileset
            tileset = (TilesetForSpriteMultiPal) addInternalResource(new TilesetForSpriteMultiPal(id + "_tileset", frameImage, wf * 8, hf * 8, sprites, compression, false));
        }
     ...
     */
	public SpriteFrameMultiPal(String id, byte[] frameImage8bpp, int wf, int hf, int timer, CollisionType collisionType, Compression compression,
            OptimizationType optType, OptimizationLevel optLevel, SpriteBoundariesPalettes spriteBoundariesPals)
    {
        this(id, frameImage8bpp, wf, hf, timer, collisionType, compression, computeSpriteCutting(id, frameImage8bpp, wf, hf, optType, optLevel), spriteBoundariesPals);
    }
     ...
     *        height of frame in tile
     */
    public SpriteFrameMultiPal(String id, byte[] image8bpp, int w, int h, int frameIndex, int animIndex, int wf, int hf, int timer, CollisionType collisionType,
            Compression compression, OptimizationType optType, OptimizationLevel optLevel, SpriteBoundariesPalettes spriteBoundariesPals)
    {
        this(id, ImageUtil.getSubImage(image8bpp, new Dimension(w * 8, h * 8), new Rectangle((frameIndex * wf) * 8, (animIndex * hf) * 8, wf * 8, hf * 8)), wf,
                hf, timer, collisionType, compression, optType, optLevel, spriteBoundariesPals);
    }
    ...
    public List<SpriteCell> getSprites()
    {
        ...
        for(VDPSpriteMultiPal sprite: vdpSprites)
    ...
    public boolean isOptimisable()
    {
        ...
        {
            final VDPSpriteMultiPal vdpSprite = vdpSprites.get(0);
    ...
    {
        if (obj instanceof SpriteFrameMultiPal)
        {
            final SpriteFrameMultiPal spriteFrame = (SpriteFrameMultiPal) obj;
