    ...
    public Resource execute(String[] fields) throws Exception
    {
        if (fields.length < 6)
        {
            ...
            System.out.println("  palsPosition         Possible values (use only one): "
            		+ PalettesPositionEnum.PAL0PAL1.getValue() + ", " 
            		+ PalettesPositionEnum.PAL2PAL3.getValue() + " . It marks the palettes location in the image.");
...
//		Compiler.addResourceFile(fileIn);
		...
		if (splitChunks == 1)
			return new Palette32AllStrips(id, stripsInList, palsPosition, togglePalsLocation, compression, compressionCustom, addCompressionField);
		else if (splitChunks == 2)
			return new Palette32AllStripsSplit2(id, stripsInList, palsPosition, togglePalsLocation, compression, compressionCustom, addCompressionField);
		else
			return new Palette32AllStripsSplit3(id, stripsInList, palsPosition, togglePalsLocation, compression, compressionCustom, addCompressionField);
    }
...
//			"22", "1", PalettesPositionEnum.PAL0PAL1.getValue(), "TRUE", "FAST", "NONE", "FALSE"
//		};
//		p.execute(fields);
//	}
}
