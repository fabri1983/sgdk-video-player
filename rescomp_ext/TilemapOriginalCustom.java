...
public class TilemapOriginalCustom extends Resource
{
	...
	private static TilemapCreationData createTilemap(String id, List<TilesetOriginalCustom> tilesets, int[] offsetPerTilesetChunk,
        ...
        if (!commonTilesRange.isEmpty()) {
			if (frameNum != null) {
				...
				System.out.println("[WARNING] Couldn't extract frameNum for CommonTilesRange from " + id + ". " + TilemapOriginalCustom.class.getSimpleName());
        	}
        }
    ...
    {
        if (data.length != (w * h))
            throw new RuntimeException(TilemapOriginalCustom.class.getSimpleName() + ".setData(..): size do not match !");
