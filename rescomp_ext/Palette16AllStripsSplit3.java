 ...
 */
public class Palette16AllStripsSplit3 extends Resource
{
    final int hc;
    ...
    public Palette16AllStripsSplit3(String id, List<String> stripsFileList, final PalettesPositionEnum palsPosition, final boolean togglePalsLocation, 
    		Compression compression, CompressionCustom compressionCustom, boolean addCompressionField) throws Exception
    {
        ...
        if (compressionCustom == CompressionCustom.RLEW_A || compressionCustom == CompressionCustom.RLEW_B) {
        	...
        	System.setProperty(bin3.id + RLEWCompressor.RLE_PROPERTY_SUFFIX_WORDS_PER_ROW, String.valueOf(16));
        }
...
//	}
    ...
    {
        if (obj instanceof Palette16AllStripsSplit3)
        {
            final Palette16AllStripsSplit3 palette = (Palette16AllStripsSplit3) obj;
    ...
    public void out(ByteArrayOutputStream outB, StringBuilder outS, StringBuilder outH)
    {
		...
		if (!MdComp.checkAllSameCompression(bin1, bin2, bin3))
			throw new RuntimeException("Resource id " + id + " has different compression methods for bin1, bin2, and bin3");

		// declare
		if (addCompressionField)
			Util.decl(outS, outH, CustomDataTypes.Palette16AllStripsSplit3CompField.getValue(), id, 2, global);
		else
			Util.decl(outS, outH, CustomDataTypes.Palette16AllStripsSplit3.getValue(), id, 2, global);
