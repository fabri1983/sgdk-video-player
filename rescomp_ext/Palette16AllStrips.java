 ...
 */
public class Palette16AllStrips extends Resource
{
    ...
    public Palette16AllStrips(String id, List<String> stripsFileList, final PalettesPositionEnum palsPosition, final boolean togglePalsLocation, 
    		Compression compression, CompressionCustom compressionCustom, boolean addCompressionField) throws Exception
    {
        ...
        if (compressionCustom == CompressionCustom.RLEW_A || compressionCustom == CompressionCustom.RLEW_B) {
        	System.setProperty(bin.id + RLEWCompressor.RLE_PROPERTY_SUFFIX_WORDS_PER_ROW, String.valueOf(16));
        }
...
//	}
    ...
    {
        if (obj instanceof Palette16AllStrips)
        {
            final Palette16AllStrips palette = (Palette16AllStrips) obj;
    ...
    public void out(ByteArrayOutputStream outB, StringBuilder outS, StringBuilder outH)
    {
		...
		// declare
		if (addCompressionField)
			Util.decl(outS, outH, CustomDataTypes.Palette16AllStripsCompField.getValue(), id, 2, global);
		else
			Util.decl(outS, outH, CustomDataTypes.Palette16AllStrips.getValue(), id, 2, global);
